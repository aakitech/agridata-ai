...
- **Client-Side Invitation Ingestion**: Due to how Supabase sends security tokens via URL hash fragments (which are invisible to the server), the `/accept-invite` page is a Client Component. It manually extracts tokens and injects the session into the Supabase client to ensure a robust onboarding experience regardless of browser timing.
